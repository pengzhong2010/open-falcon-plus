...
import (
	"encoding/json"
	bi "github.com/favframework/debug"
	"github.com/fsnotify/fsnotify"
	"github.com/hpcloud/tail"
	"github.com/pengzhong2010/open-falcon-plus/falcon-logdog/config"
	"github.com/pengzhong2010/open-falcon-plus/falcon-logdog/log"
...
func readFileAndSetTail(file *config.WatchFile) {
	...
	tail, err := tail.TailFile(file.ResultFile.FileName, tail.Config{Follow: true, Location: &tail.SeekInfo{Offset: 0, Whence: 2}})
	if err != nil {
		log.Fatal(err)
	}
...
func handleKeywords(file config.WatchFile, line string) {
	//bi.Dump("----handleKeywords----")
	//bi.Dump(file.Keywords)
	for _, p := range file.Keywords {
		...
		if v, ok := keywords.Get(p.Tag); ok {
			...
			data = config.PushData{Metric: config.Cfg.Metric,
				...
				CounterType: "GAUGE",
				Tags:        "type=log" + ",prefix=" + file.Prefix + ",suffix=" + file.Suffix + "," + p.Tag + "=" + p.FixedExp,
			}
		}

		keywords.Set(p.Tag, data)
	...
	for _, v := range c.WatchFiles {
		for _, p := range v.Keywords {
			...
			data := config.PushData{Metric: c.Metric,
				...
				Step:        c.Timer,
				CounterType: "GAUGE",
				Tags:        "type=log" + ",prefix=" + v.Prefix + ",suffix=" + v.Suffix + "," + p.Tag + "=" + p.FixedExp,
			}
