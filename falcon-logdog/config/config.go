	...
	go func() {
		for {
			...
			case event := <-watcher.Events:
				compare_configfile := "./" + configFile
				if event.Name == compare_configfile && (event.Op == fsnotify.Chmod || event.Op == fsnotify.Rename || event.Op == fsnotify.Write || event.Op == fsnotify.Create) {
